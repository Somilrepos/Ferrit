...
pub(crate) fn init_at(root: &Path) -> Result<(), String> {
    ...
    fs::rename(temp_ferrit_dir, ferrit_dir)
        .map_err(|e| format!("Failed to finalize repository initialization: {e}"))?;
